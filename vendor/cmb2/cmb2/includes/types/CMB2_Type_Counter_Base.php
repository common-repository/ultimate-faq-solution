...
abstract class CMB2_Type_Counter_Base extends CMB2_Type_Base {
	...
	public function char_counter_markup( $val ) {
		...
		switch ( $type ) {
			case 'words':
				$label = $char_max
					? $this->_text( 'words_left_text', esc_html__( 'Words left', 'ufaqsw' ) )
					: $this->_text( 'words_text', esc_html__( 'Words', 'ufaqsw' ) );
				break;
			default:
				$type  = 'characters';
				$label = $char_max
					? $this->_text( 'characters_left_text', esc_html__( 'Characters left', 'ufaqsw' ) )
					: $this->_text( 'characters_text', esc_html__( 'Characters', 'ufaqsw' ) );
				break;
		}

		$msg = $char_max
			? sprintf( '<span class="cmb2-char-max-msg">%s</span>', $this->_text( 'characters_truncated_text', esc_html__( 'Your text may be truncated.', 'ufaqsw' ) ) )
			: '';
